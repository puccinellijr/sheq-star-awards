...
export function useCertificateGenerator() {
  ...
  const generateCertificate = async (
    ...
    try {
      ...
      // Collaborator name (placeholder - would get from database)
      ctx.font = 'bold 28px Arial';
      ctx.fillStyle = '#1e40af';
      ctx.fillText('[NOME DO COLABORADOR]', canvas.width / 2, 280);
      ...
      ctx.font = '20px Arial';
      ctx.fillStyle = '#374151';
      ctx.fillText(`Referente ao mês de ${monthName} de ${year}`, canvas.width / 2, 330);

      // Recognition text
      ctx.font = '16px Arial';
      ctx.fillText('Pelo comprometimento e excelência em', canvas.width / 2, 380);
      ctx.fillText('Segurança, Saúde, Meio Ambiente e Qualidade', canvas.width / 2, 405);

      // Date and signature area
      const currentDate = new Date().toLocaleDateString('pt-BR');
      ctx.font = '14px Arial';
      ctx.fillStyle = '#6b7280';
      ctx.fillText(`São Paulo, ${currentDate}`, canvas.width / 2, 480);
      ...
      ctx.fillText('Direção Odfjell Terminals', canvas.width / 2, 540);
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          link.href = url;
          link.download = `certificado-sheq-${type}-${month}.png`;
